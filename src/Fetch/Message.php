...
class Message
{
    ...
    public static function getParametersFromStructure($structure)
    {
		...
		self::normalizeFilenameParameters($parameters);
        return $parameters;
    }

	/**
	 * This function normalizes the filename parameters in the given array. It will remove the UTF-8 prefix and decode
	 * the value.
	 *
	 * @param array $parameters The parameters array to normalize.
	 */
	private static function normalizeFilenameParameters(array &$parameters)
	{
		foreach (['filename', 'name'] as $paramBase) {
			$extendedKey = $paramBase . '*';
			if (isset($parameters[$extendedKey])) {
				// Entferne UTF-8'' Prefix
				$value = $parameters[$extendedKey];
				if (stripos($value, "utf-8''") === 0) {
				...
				// Optional: hier könnte man noch charset-decoding machen, falls nötig.
				// In der Praxis reicht bei UTF-8 meist rawurldecode.

				// Schreibe zusätzlich den "normalen" Key → kompatibel mit bestehendem Code
				$parameters[$paramBase] = $decodedValue;
			}
		}
	}
    ...
    protected function processAddressObject($addresses)
    {
        ...
        if (is_array($addresses))
            foreach ($addresses as $address) {
                if (property_exists($address, 'mailbox') && property_exists($address, 'host') && !empty($address->host)) {
